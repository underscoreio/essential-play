...
`Html` objects are lightweight wrappers for content generated by the template. Play is able to convert them to `Results` of `Content-Type: text/html`---we rarely interact with them directly from Scala code:
